...
# 新增：创建注意力权重保存目录
ATTENTION_SAVE_DIR = os.path.join(opt.checkpoint, "attention_weights")
...
def val(opt, actions, val_loader, model, epoch):
    with torch.no_grad():
        return step('test', opt, actions, val_loader, model, epoch=epoch)  # 新增epoch参数用于保存
...
def step(split, opt, actions, dataLoader, model, optimizer=None, epoch=None):
    ...
    for i, data in enumerate(tqdm(dataLoader, 0)):
        ...
        if split == 'train':
            ...
            output_3D[:, :, 0, :] = 0
            action_error_sum = test_calculation(output_3D_single, out_target, action, action_error_sum, opt.dataset,
                                                subject)

            if opt.refine:
                output_3D[:, :, 0, :] = 0
                action_error_sum_refine = test_calculation(output_3D_single, out_target, action,
                                                           action_error_sum_refine, opt.dataset, subject)

            # 新增：保存最后一个STC块的注意力权重（每个epoch保存一次）
            if epoch is not None:
                attn_weights = model_trans.last_attn_weights  # 从Model实例获取
                if attn_weights is not None:
                    save_path = os.path.join(ATTENTION_SAVE_DIR, f"attention_epoch_{epoch}_batch_{i}.pt")
                    torch.save(attn_weights.cpu(), save_path)
...
if __name__ == '__main__':
    ...
    for epoch in range(1, opt.nepoch):
        ...
        # 传入epoch参数用于保存注意力权重
        p1, p2 = val(opt, actions, test_dataloader, model, epoch=epoch)
